...
function cleanContent(text) {
  ...
  for (const line of lines) {
    if (line.startsWith('Popular Posts') || line.startsWith('Categories') || 
        line.startsWith('Tags') || line.match(/^- [Α-Ω]{2,}/) ||
        line.includes('Φεβρουαρίου 07, 2024') || line.includes('Απριλίου 26, 2023') ||
        line.includes('Ιανουαρίου 03, 2024') || line.startsWith('Επιστημονική Δημοσίευση στο')) {
      break;
    }
    clean.push(line);
  }
...
function toHtml(lines) {
  return lines.map(line => {
    ...
    if (t.startsWith('- ')) return `  <p>${t.substring(2)}</p>`;
    return `  <p>${t}</p>`;
  }).join('\n');
}
...
for (const [txtFile, astroFile] of mapping) {
  try {
    if (!fs.existsSync(txtFile)) { skipped++; continue; }
    ...
    // Extract frontmatter and opening PostLayout tag (everything up to and including the > after tags)
    const closeTag = astro.indexOf('\n>', astro.indexOf('<PostLayout'));
    if (closeTag === -1) { console.log('SKIP ' + astroFile + ' - no PostLayout'); skipped++; continue; }
    
    const header = astro.substring(0, closeTag + 2); // includes the \n>
    const cleanLines = cleanContent(content);
    const htmlContent = toHtml(cleanLines);
    
    const newAstro = header + '\n' + htmlContent + '\n</PostLayout>\n';
    fs.writeFileSync(astroFile, newAstro, 'utf-8');
    updated++;
    console.log('OK ' + path.basename(astroFile).padEnd(55) + cleanLines.length + ' paragraphs');
  } catch(e) {
    console.log('ERR ' + astroFile + ': ' + e.message);
    skipped++;
  }
}
